...
from constants import clean_name, PATH_LEMMA, PATH_CLUSTER, PATH_THEME, PATH_IMGS_CODO, PATH_IMGS_CLUSTER

def clustering_module(modulo, p_max_df, p_min_df):

    file_name = clean_name(modulo)

    csv_name_lemma = PATH_LEMMA + "/" + file_name + ".csv"
    csv_name_cluster = PATH_CLUSTER + "/" + file_name + ".csv"
    csv_name_themes = PATH_THEME + "/" + file_name + ".csv"
    ...
    print("Loading data...")
    df_incidencias_lemmas = pd.read_csv(csv_name_lemma)
    ...
    nombre_archivo_codo = ombre_archivo_grafico = PATH_IMGS_CODO + "/" + img_name #f"codo_modulo_{modulo.replace(' ', '_')}.png"
